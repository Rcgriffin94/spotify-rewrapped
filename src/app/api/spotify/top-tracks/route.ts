...
export async function GET(request: NextRequest) {
  try {
    ...
    if (timeRange === 'custom') {
      ...
      // Check if date range is too far in the past (Spotify only keeps ~50 recent tracks)
      ...
      // For custom ranges, we'll need to simulate by getting recently played tracks
      // and filtering by date range, then calculating top tracks from that data
      return NextResponse.json(
        { 
    ...
    return NextResponse.json(
      { 
        error: 'Failed to fetch top tracks. Please try again.',
        code: 'INTERNAL_ERROR'
      },
      { status: 500 }
    );
